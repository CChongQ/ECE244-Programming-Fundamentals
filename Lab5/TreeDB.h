...
private:
   TreeNode* root;
   int probesCount;
   bool insert_bst(TreeNode* r, DBentry* newEntry);
   void clear_bst(TreeNode* r);
   DBentry* find_bst(string name,TreeNode* tempTN);
   TreeNode* remove_re(TreeNode* node);
   bool remove_bst(string name,TreeNode* deleteNode, TreeNode* deletePre,int a);
   void printNode(TreeNode* newNode);
   void countAC( TreeNode* node,int &a) const;
